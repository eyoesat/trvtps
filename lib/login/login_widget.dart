...
import '../login_error/login_error_widget.dart';
              ...
              return FFButtonWidget(
                onPressed: () async {
                  ...
                  if (((returned?.statusCode ?? 200)) == 200) {
                    ...
                    setState(() => FFAppState().role = getJsonField(
                          (returned?.jsonBody ?? ''),
                          r'''$.userData.role''',
                        ).toString());
                    setState(() => FFAppState().statusa = getJsonField(
                          (returned?.jsonBody ?? ''),
                          r'''$.userData.status''',
                        ).toString());
                    ...
                    await Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => LoginErrorWidget(),
                      ),
                    );
                    if (_shouldSetState) setState(() {});
                    return;
